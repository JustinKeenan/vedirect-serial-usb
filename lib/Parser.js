...
const defaults = {
  ...
  defaultUnitId: 'victronDevice',  //device specific
  mainBatt: 'house', //device specific
  auxBatt: 'starter', //device specific
  solar: 'solar' //device specific
}
...
class VEDirectParser extends EventEmitter {
  constructor (opts) {
    ...
    this.debug = debug('signalk-vedirect-parser')  //device specific
    this.line = []
    this.data = {}
    this.cache = ''
    this.sum = 0
  ...
  //incomming data may not always be ASCII - change to allow control of this option
  addChunk (buf, items) {
  ...
  //incomming data my not be in a string, should be left to output formatting to adjust this
  parse (line) {
